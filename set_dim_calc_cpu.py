#Numpy, sklearn, tqdm, and matplotlib aren't native to python, so you will need to install them
#I recommend using anaconda instead of pip, this will resolve many headaches when installing Nvidia drivers and CUDA toolkits
import numpy as np
...
def mandelbrot_set_cpu(res_c, res, res_tile, iter_scale, growth_rate):
    ########parameters##########
    ...
    while res_c < res:
        ...
        if res_c < res_tile:
            ...
            #these nested loops loop over the logic set (which is now on the cpu) to find the amount of points in the set
            ...
            while tile_stop_c <= tile_stop:
                ...
                while tile_c_y < tile_stop_c:
                    while tile_c_x < tile_stop_c:
                        ...
                        X = np.linspace((-2+(step_tile*tile_c_x)), (-2+(step_tile*(tile_c_x+1))), num=res_tile).reshape((1, res_tile))
                        Y = np.linspace((-2+(step_tile*tile_c_y)), (-2+(step_tile*(tile_c_y+1))), num=res_tile).reshape((res_tile, 1))
                        C = np.tile(X, (res_tile, 1)) + 1j * np.tile(Y, (1, res_tile))

                        Z = np.zeros((res_tile, res_tile), dtype=complex)
                        M = np.full((res_tile, res_tile), True, dtype=bool)
                        ...
                        print("Now finding the set")
                        #these nested loops loop over the logic set (which is now on the cpu) to find the amount of points in the set
                        for i in tqdm(range(res_tile)):
                            for j in range(res_tile):
                                if M[i, j] == True:
                                    point_c = point_c + 1
